...
5. [本番公開](#本番公開)
   - [Cloudflare Tunnel（自宅 / ローカル環境向け）](#cloudflare-tunnel自宅--ローカル環境向け)
   - [Caddy / Nginx（VPS 向け）](#caddy-例)
6. [運用](#運用)
...
Docker イメージは GHCR で公開されているため、ホスト側に Node.js / pnpm / Vite+ は不要です。
...
GHCR で公開されているイメージを使う方法と、ローカルでビルドする方法があります。

```bash
# GHCR の公開イメージを使う場合 (推奨)
...
GitHub Actions から到達可能にするには、HTTPS エンドポイントが必要です。トンネルサービスを使う方法と、リバースプロキシを自前で立てる方法があります。

### Cloudflare Tunnel（自宅 / ローカル環境向け）

VPS やドメインを持っていなくても、トンネルサービスを使えばローカルマシンから HTTPS エンドポイントを公開できます。リバースプロキシや TLS 証明書の設定は不要です。
...
# トンネル開始 (ランダムな URL が発行される)
cloudflared tunnel --url http://localhost:3000
```

ターミナルに表示される `https://xxx.trycloudflare.com` を `CODEX_REVIEW_URL` として GitHub Secrets に設定してください。

> **注意**: クイックトンネルは `cloudflared` プロセスを停止すると URL が無効になり、再起動のたびに URL が変わります。テストや個人利用には十分ですが、常用する場合は Named Tunnel を使ってください。

#### Named Tunnel（常用向け）

Cloudflare アカウントに紐づけた固定サブドメインで公開できます。
...
# トンネルを作成 (名前は任意)
cloudflared tunnel create codex-review

# DNS レコードを追加 (例: review.example.com)
cloudflared tunnel route dns codex-review review.example.com
```
...
```yaml
tunnel: <トンネル ID>
credentials-file: ~/.cloudflared/<トンネル ID>.json
...
```bash
# トンネルをバックグラウンドで起動
cloudflared tunnel run codex-review
```

> **Tips**: `cloudflared` を systemd / launchd サービスとして登録すれば、マシン再起動時にも自動的にトンネルが復旧します。
>
> ```bash
> # macOS (launchd)
