...
describe("Store", () => {
	describe("threads", () => {
		...
		it("upserts on duplicate thread_id", () => {
			store.insertThread({ threadId: "t1", repo: "acme/app", sha: "aaa", kind: "push", createdAt: 1 });
			store.insertThread({ threadId: "t1", repo: "acme/app", sha: "bbb", kind: "push", createdAt: 2 });
			const t = store.getThread("t1");
			expect(t!.sha).toBe("bbb");
		});

		it("handles optional fields (sha, number)", () => {
			store.insertThread({ threadId: "t2", repo: "acme/app", kind: "issues", number: 42, createdAt: 1 });
